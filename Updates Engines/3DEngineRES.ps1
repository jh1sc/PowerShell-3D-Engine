...
SetFontAsp 0 0 7 53 400 Consolas
...
while ($true) {
    ...
    for ($t = 0; $t -lt $Faces.length; $t++) {
        ...
        if ($dot -lt 0) {
            ...
            $y1 = [math]::round((($y1 * ($Fov / ($Fov + $z1))) + $nScreenHeight / 2)/2)
            $x2 = [math]::round(($x2 * ($Fov / ($Fov + $z2))) + $nScreenWidth / 2)
            $y2 = [math]::round((($y2 * ($Fov / ($Fov + $z2))) + $nScreenHeight / 2)/2) 
            $x3 = [math]::round(($x3 * ($Fov / ($Fov + $z3))) + $nScreenWidth / 2)
            $y3 = [math]::round((($y3 * ($Fov / ($Fov + $z3))) + $nScreenHeight / 2)/2)
            if ($trifill -eq $true) {
